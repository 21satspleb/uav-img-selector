...
with col2:
  ...
  if image_path in st.session_state.uav_images.image_dict:
      ...
      if lat_lon is not None:
          # Create the map
          map_ = folium.Map(location=[lat_lon[0], lat_lon[1]], zoom_start=13)
          # Add the marker
          folium.Marker(location=[lat_lon[0], lat_lon[1]], popup="Selected Image").add_to(map_)
          # Display the map in Streamlit
          with col2_1:
              st.image(image, caption=image_path, width=600)
          with col2_2:
